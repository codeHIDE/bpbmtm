...
<html>
  ...
  <body>
  	<form action="statistics!dayMerAmtProfit.ac" method="post" id="from1">
  			...
  			<tr>
  				...
  				<td style="text-align: left;width: 100px">
  					<select name="merType"  id="merType">
  						...
  						<option value="2" <c:if test="${merType=='2'}">selected="selected"</c:if>>1级代理</option>
  						<option value="3" <c:if test="${merType=='3'}">selected="selected"</c:if>>2级代理</option>
  						<option value="4"<c:if test="${merType=='4'}">selected="selected"</c:if>>平台类商户</option>
  					</select>
  				</td>
  				<td style="text-align: right;">
					商户交易时间：
