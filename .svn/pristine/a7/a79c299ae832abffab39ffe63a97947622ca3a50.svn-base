...
<script type="text/javascript">
        ...
        function loadData() {
            ...
            { display: '商户号', name: 'subMerId', width: '10%', align: 'center' },
            { display: '终端号', name: 'terminalId', width: '10%', align: 'center' },
            { display: '代付金额', name: 'payAmt', width: '6%', align: 'right', type:'currency'}, 
            { display: '代付状态', name: 'settleStatus', width: '5%', align: 'center',render:function(rowdata,rowindex){
            	if(rowdata.settleStatus=='1'){return '代付成功';}
            	else if(rowdata.settleStatus=='3'){return '代付失败';}
            	else if(rowdata.settleStatus=='4'){return '处理失败';}
            	else if(rowdata.settleStatus=='5'){return '代付中';}
            } }, 
						  ...
						  if(rowdata.settleStatus<=0 && rowdata.respCode =="00"){
						   ...
						   if(rowdata.settleStatus>0 && rowdata.respCode =="00"){
	 						h += " <a onclick='failedOrder("+rowdata.orderId+",\""+rowdata.merOrderId+"\")' style='cursor:pointer;color:blue;'><span class='span_show' style='visibility:visible'>标记失败</span></a> ";
	 						  ...
	 						  if((rowdata.orderRateType=="10" || rowdata.orderRateType=="15") && rowdata.respCode =="00"){
	 						h += " <a onclick='searchPay("+rowdata.orderId+",\""+rowdata.merOrderId+"\")' style='cursor:pointer;color:blue;'><span class='span_show' style='visibility:visible'>代付查询</span></a> ";
							 }
						</c:if>
        ...
        function searchPay(orderId,merOrderId)
        {
           window.parent.$.ligerDialog.confirm('是否查询此订单?', function (yes) {
	           if(yes){
		     		$(".span_show").css("visibility","hidden");
				     $.ajax({
			                   type: "POST",
			                   dataType: "text",
			                   url: '<s:url value="/orderInfo!searchPay.ac"/>',
			                   data: {"orderId":orderId,"merOrderId":merOrderId},
			                   success: function (data) {
			                     alert(data);
			                     // window.parent.$.ligerDialog.closeWaitting();
