...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head><meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
		...
		<script type="text/javascript">
			...
			function check(){
			if(len("loginName",20,"EN","授权人ID")&&len("loginPwd",32,"EN","级别确认密码")){
				if(confirm("是否确认操作？"))
					$.post("<s:url value='/subMerInfo!selectSubMerInfoPass.ac'/>",
					{'sysManager.levelPwd':$("#loginPwd").val(),'sysManager.loginName':$("#loginName").val()},
					function(data){
						if(data=='succ'){
							parent.$.ligerDialog.close();
							parent.$.ligerDialog.open({ url: '<s:url value="/subMerInfo!selectUpdateSettSubMerchant.ac"/>?id='+$("#subMerId").val(),
	            			 height:300,width:600, isResize: false,title:'结算信息修改'});
						}else{
							alert("失败！");
						}
					},"text");
				...
				<table width="100%">
					<tr>
						...
						<td>
							...
							<input value=<s:property value="id" /> type="hidden" id="subMerId"/>
						</td>
					</tr>
					<tr>
						<td>级别确认密码：</td>
						<td><input name="sysManager.levelPwd" value="" type="password" id="loginPwd" style="width:150px"/></td>
					</tr>
					<tr>
