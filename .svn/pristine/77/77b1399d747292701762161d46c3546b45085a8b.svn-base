        ...
        this.each(function()
        {
            ...
            var g = {
                ...
                addTabItem: function(options)
                {
                    ...
                    var showClose = options.showClose;
                    var height = options.height;
                   	if(g.getTabItemCount()>10){
                   		alert("您打开的窗口过多,请关闭您不需要的窗口。");
                   		return;
                   	}
                    //如果已经存在
                    if (g.isTabItemExist(tabid))
                ...
                removeTabItem: function(tabid)
                {
                    ...
                    if (currentIsSelected)
                    {
                        ...
                        $("li[tabid=" + tabid + "]", g.tab.links.ul).prev().addClass("l-selected").siblings().removeClass
