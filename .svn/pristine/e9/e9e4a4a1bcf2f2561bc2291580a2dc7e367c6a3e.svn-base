	...
	<div id="top">
		...
		<ul class="nav">
			<li>
				...
				<li>
					<a href="<s:url value="/orderInfo!orderIn.ac" />"
		...
		<ul class="sub">
			...
			<c:if test="${currentModule == 10}">	
					<li>
						<a
							href="<s:url value="/merInfoIndex!selectMerchantInfoIndex.ac" />">商户详情</a><span>丨</span>
					</li>
				
					<li>
						<a href="<s:url value="/selectSubMerInfo!selectSubMerchantInfoListJoin.ac" />">子商户查询</a><span>丨</span>
					</li>
			</c:if>
			<!-- 交易管理 --> 
			<c:if test="${currentModule == 40}">
					<li>
						<a href="<s:url value="/orderInfo!selectTheDaoOrderInfoList.ac" />">当天交易查询</a><span>丨</span>
					</li>
