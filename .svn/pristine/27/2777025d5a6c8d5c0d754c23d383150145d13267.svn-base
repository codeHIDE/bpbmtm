...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<script type="text/javascript">
           	 	   ...
           	 	   function f_search()
        {
           ...
           { display: '操作', name: 'null', width: '10%', align: 'center' ,render: function (rowdata, rowindex, value)
           	 	{
           	 		 var h = "";
		             <c:if test="${purview['8061'] == '8061'}">
              		  	h = "<a onclick='delectMerModel("+rowdata.id+")' style='cursor:pointer;color:blue;'>删除</a> ";
              		 </c:if>
              		 <c:if test="${purview['8062'] == '8062'}">
              		 	h += "<a onclick='updateMerModel("+rowdata.id+")' style='cursor:pointer;color:blue;'>编辑</a> ";
              		 </c:if>
                    return h;
                }}
               ],  
               rownumbers:true,
                ...
                dataAciton : 'server',    
                url:'<s:url value="/merModel!selectMerModelList.ac"/>'                           
            });
           ...
           function addWindow(){
            $.ligerDialog.open({ url: 'addMerModel.jsp',
            	 height:270,width: 420, isResize: false,title:'权限添加'});    
        	}
        	
           function updateMerModel(id){
            $.ligerDialog.open({ url: '<s:url value="/merModel!updateMerModelInit.ac"/>?merModel.id='+id,
            	 height:270,width: 420, isResize: false,title:'权限修改'});    
        	}
        	
        
             function delectMerModel(id){
       		 $.ligerDialog.confirm('是否要删除?', function (yes) {
        	if(yes){
        		$.post('<s:url value='/merModel!deleteMerModel.ac'/>',
				  {
				  		 'merModel.id':id
				  },
				  function(data){
	...
	<form id="form1" action="<s:url value="/pages/codePage/merModelList.jsp"/>" method="post">
				<table width="80%">
				</table>
			</form>
