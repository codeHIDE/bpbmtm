			...
			<div position="left" title="主要菜单" id="accordion1">
				...
				<c:if test="${purview['200'] == '200'}">
					<div title="交易管理">
	                    ...
	                    <c:if test="${purview['201'] == '201'}">
	                  	...
	                  	<c:if test="${purview['2002'] == '2002'}">
	                   		<a class="l-link" href="javascript:f_addTab('t1pay','t+1打款','<s:url value="/pages/order/t1pay.jsp"/>')">t+1打款</a>
	                  	</c:if>
	                  	<c:if test="${purview['2003'] == '2003'}">
	                   		<a class="l-link" href="javascript:f_addTab('coreTransLog','核心交易查询','<s:url value="/pages/order/coreTransLog.jsp"/>')">POS交易查询</a>
	                  	</c:if>
	                  	<c:if test="${purview['2004'] == '2004'}">
	                   		<a class="l-link" href="javascript:f_addTab('dyPayInfo','代付查询','<s:url value="/pages/order/dyPay.jsp"/>')">代付查询</a>
	                  	</c:if>
	                  	<c:if test="${purview['2005'] == '2005'}">
	                   		<a class="l-link" href="javascript:f_addTab('scanPayInfo','快捷交易查询','<s:url value="/pages/order/scanPay.jsp"/>')">快捷交易查询</a>
	                  	</c:if>
                ...
                <c:if test="${purview['300'] == '300'}">
					...
					<div style="height: 7px;"></div>
						...
						<c:if test="${purview['308'] == '308'}">
							<a class="l-link" href="javascript:f_addTab('countMerPuisneTerminal','机构下属终端统计','<s:url value="/pages/statistics/countMerPuisneTerminal.jsp"/>')">机构下属终端统计</a>
						</c:if>
						<c:if test="${purview['309'] == '309'}">
							<a class="l-link" href="javascript:f_addTab('sumMerProfit','机构分润统计','<s:url value="/pages/statistics/sumMerProfit.jsp"/>')">机构分润统计</a>
						</c:if>
						<c:if test="${purview['310'] == '310'}">
							<a class="l-link" href="javascript:f_addTab('merProfitApply','机构提现查询','<s:url value="/pages/statistics/merProfitApply.jsp"/>')">机构提现查询</a>
						</c:if>
					</div>
				</c:if>
...
<%--						</c:if>--%>
						...
						<c:if test="${purview['410'] == '410'}">
							<a class="l-link" href="javascript:f_addTab('getTodaySubMerCashOut','T+0结算查询','<s:url value="/MerSettleStatictis!getTodaySubMerCashOut.ac"/>')">T+0结算查询</a>
						</c:if>
						<c:if test="${purview['411'] == '411'}">
							<a class="l-link" href="javascript:f_addTab('checkXls','xls清算','<s:url value="/pages/theReport/checkStatic.jsp"/>')">Xls清算</a>
						</c:if>
						<c:if test="${purview['412'] == '412'}">
							<a class="l-link" href="javascript:f_addTab('T1checkFile','T1报盘文件下载','<s:url value="/pages/theReport/checkReport.jsp"/>')">T1报盘文件下载</a>
						</c:if>
						<c:if test="${purview['413'] == '413'}">
							<a class="l-link" href="javascript:f_addTab('checkPos','Pos对账','<s:url value="/pages/theReport/checkPos.jsp"/>')">Pos对账</a>
						</c:if>
					</div>
				</c:if>
				...
				<c:if test="${purview['600'] == '600'}">
					...
					<div style="height: 7px;"></div>
						...
						<c:if test="${purview['607'] == '607'}">
						<a class="l-link" href="javascript:f_addTab('cloneTerminal','替换机具','<s:url value="/pages/subMerTerminal/cloneTerminal.jsp"/>')">替换机具</a>
						</c:if>
						<c:if test="${purview['608'] == '608'}">
						<a class="l-link" href="javascript:f_addTab('mobileUser','手机用户查询','<s:url value="/pages/subMerTerminal/selectMobileUser.jsp"/>')">手机用户查询</a>
						</c:if>
					</div>
				</c:if>
				<c:if test="${purview['700'] == '700'}">
					...
					<div style="height: 7px;"></div>
						...
						<c:if test="${purview['706'] == '706'}">
						<a class="l-link" href="javascript:f_addTab('alertSubMer','提示商户','<s:url value="/pages/BlackInfo/alertSubMer.jsp"/>')">提示商户</a>
						</c:if>
						<c:if test="${purview['707'] == '707'}">
						<a class="l-link" href="javascript:f_addTab('idCardCheck','身份证验证','<s:url value="/pages/BlackInfo/idCardCheck.jsp"/>')">身份证验证</a>
						</c:if>
						<c:if test="${purview['708'] == '708'}">
						<a class="l-link" href="javascript:f_addTab('selectIdcard','身份证查询','<s:url value="/pages/BlackInfo/selectIdcard.jsp"/>')">身份证查询</a>
						</c:if>
					</div>
				</c:if>
				...
				<c:if test="${purview['1100'] == '1100'}">
					<div title="会员管理">	
					<div style="height: 7px;"></div>
					<c:if test="${purview['1101'] == '1101'}">
						<a class="l-link" href="javascript:f_addTab('selectUserType','会员等级设置','<s:url value="/pages/member/selectUserType.jsp"/>')">会员等级设置</a>
						</c:if>
					</div>
				</c:if>
				<c:if test="${purview['1200'] == '1200'}">
					<div title="商品管理">	
					<div style="height: 7px;"></div>
					<c:if test="${purview['1201'] == '1201'}">
						<a class="l-link" href="javascript:f_addTab('selectWares','商品查询','<s:url value="/pages/wares/selectWares.jsp"/>')">商品查询</a>
						</c:if>
					</div>
				</c:if>
				<c:if test="${purview['1300'] == '1300'}">
					<div title="POS渠道管理">	
					<div style="height: 7px;"></div>
					<c:if test="${purview['1301'] == '1301'}">
						<a class="l-link" href="javascript:f_addTab('selectChannel','渠道地址管理','<s:url value="/pages/channel/selectChannel.jsp"/>')">渠道地址管理</a>
					</c:if>
					<c:if test="${purview['1302'] == '1302'}">
						<a class="l-link" href="javascript:f_addTab('selectChannelMer','渠道商户管理','<s:url value="/pages/channel/selectChannelMer.jsp"/>')">渠道商户管理</a>
					</c:if>
					<c:if test="${purview['1303'] == '1303'}">
						<a class="l-link" href="javascript:f_addTab('selectChannelFeeType','渠道费率管理','<s:url value="/pages/channel/selectChannelFee.jsp"/>')">渠道费率管理</a>
					</c:if>
					<c:if test="${purview['1304'] == '1304'}">
						<a class="l-link" href="javascript:f_addTab('addChannelMer','添加渠道商户','<s:url value="channel!addChannelMer.ac"/>')">添加渠道商户</a>
					</c:if>
					</div>
				</c:if>
