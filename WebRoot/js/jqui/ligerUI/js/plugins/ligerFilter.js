...
(function ($)
{
    ...
    $.ligerui.controls.Filter.ligerExtend($.ligerui.core.UIComponent, {
        ...
        getData: function (group)
        {
            ...
            $("> tbody > tr", group).each(function (i, row)
            {
                ...
                else
                {
                    ...
                    var tractId = $(".tractId:first", row).val();
                    var ratesType = $(".ratesType:first", row).val();
                    var amount = g._getRuleValue(row, field,1);
                    var value = g._getRuleValue(row, field,2);
                    var type = $(row).attr("fieldtype") || "string";
                    if (!groupData.rules) groupData.rules = [];
                    groupData.rules.push({
                    field: fieldName,amount:amount, op: op,ratesType:ratesType,tractId:tractId
                    	/*
                        field: fieldName,amount:amount, op: op, value: value, type: type
                        */
                    });
                }
            });
        ...
        _getRuleValue: function (rulerow, field,status)
        {
            var g = this, p = this.options;
            var editorid = $(rulerow).attr("editorid");
            ...
            if(status==1){
            	return $(".valAmount:first", rulerow).val();
            }else if(status==2){
        ...
        _bulidGroupTableHtml: function (altering, allowDelete)
        {
            ...
            //add group  注释掉添加组件 的下拉框
            tableHtmlArr.push('<input type="button" value="' + p.strings['addgroup'] + '" class="addgroup">');
           	*/
            //add rule
            tableHtmlArr.push('<font style="color:red">说明：请至少选择1、2、3中的1种条件，再请选择通道类型</font>');
        ...
        _bulidRuleRowHtml: function (fields)
        {
            ...
            rowHtmlArr.push('1、<font style="color:red">金额</font><select class="fieldsel">');
            for (var i = 0, l = fields.conditions.length; i < l; i++)
            {
                var field = fields.conditions[i];
            ...
            //添加一个金额的输入框
			rowHtmlArr.push('<td class="l-filter-amount">');
			rowHtmlArr.push('<input type="text" class="valAmount" />元');
			rowHtmlArr.push('</td>');
			
			
            rowHtmlArr.push('<td class="l-filter-op">');
            rowHtmlArr.push('2、<font style="color:red">卡类型</font><select class="opsel">');
            rowHtmlArr.push(g._bulidOpSelectOptionsHtml(fieldType));
            rowHtmlArr.push('</select>');
            rowHtmlArr.push('</td>');
            
            //费率类型
            rowHtmlArr.push('<td class="l-filter-ratesType">');
            rowHtmlArr.push('3、<font style="color:red">费率类型</font><select class="ratesType">');
            rowHtmlArr.push('<option value="">--请选择--</option>');
            rowHtmlArr.push('<option value="01">扣率型</option>');
            rowHtmlArr.push('<option value="03">封顶型</option>');
            rowHtmlArr.push('</select>');
            rowHtmlArr.push('</td>');
            ...
            */
            //添加一个通道列表框
			rowHtmlArr.push('<td class="l-filter-amount">');
			rowHtmlArr.push('<font style="color:red">通道<select class="tractId">');
			rowHtmlArr.push('<option value="">--请选择--</option>');
            for (var i = 0, l = fields.conditions2.length; i < l; i++)
            {
                var field = fields.conditions2[i];
                rowHtmlArr.push('<option value="' + field.display + '"');
                //if (i == 0) rowHtmlArr.push(" selected ");
                rowHtmlArr.push('>');
